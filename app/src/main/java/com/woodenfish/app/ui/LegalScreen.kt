...
fun UserAgreementScreen(onBack: () -> Unit) {
    Column(
        ...
        Section(title = "1. 服务说明") {
            Text(body = "Doki（以下简称本应用）是一款电子木鱼应用，提供敲击木鱼、功德计数、定时提醒、电子求签与掷骰子功能，旨在为用户提供减压、冥想辅助和娱乐功能。本应用所有功能均为免费提供。")
        }
        ...
        Section(title = "3. 免责声明") {
            Text(body = "本应用提供的计数、提醒、求签、掷骰子等功能仅供娱乐参考，不构成任何形式的建议或承诺。求签结果与骰子点数均由程序随机生成，与任何现实运势、命理或决策无关，请勿据此做出重要决定。开发者不对因使用本应用而产生的任何直接或间接损失承担责任。")
        }
